...
class SmartWoo_Admin_Controller {
	 ...
	 */
	public static function new_service_form() {
		...
		if ( isset( $_POST['add_new_service_submit'], $_POST['sw_add_new_service_nonce']  ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['sw_add_new_service_nonce'] ) ), 'sw_add_new_service_nonce' ) ) {
			...
			if ( ! empty( $validation_errors ) ) {
				...
				wp_redirect( admin_url( 'admin.php/?page=sw-admin&tab=add-new-service' ) );
				exit;
			}
			...
			$details_url      = admin_url( 'admin.php?page=sw-admin&tab=view-service&service_id=' . $saved_service_id );
	 ...
	 */
	public static function edit_service_form() {
